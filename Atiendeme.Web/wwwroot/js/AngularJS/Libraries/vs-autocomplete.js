        ...
        return {
            ...
            link: function (scope, element, attrs, ctrls) {
                ...
                google.maps.event.addListener(autocomplete, 'place_changed', function () {
                    ...
                    viewValue = place.formatted_address ? (place.formatted_address.indexOf(place.name) === -1 ? place.name + ', ' + place.formatted_address : place.formatted_address) || modelCtrl.$viewValue : modelCtrl.$viewValue;
                ...
                element.on('blur', function (event) {
                    //Change so we can get the name of the place plus the formatted address
                    viewValue = modelCtrl.$viewValue ? (place && place.formatted_address) ? viewValue ? viewValue.indexOf(place.name) === -1 ? viewValue : place.name + ', ' + viewValue : modelCtrl.$viewValue : "" : "";
                    $timeout(function () {
                        scope.$apply(function () {
                            modelCtrl.$setViewValue(viewValue);
                            modelCtrl.$render();
